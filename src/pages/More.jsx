...
import { Landmark, ShieldCheck, TrendingUp, FolderLock, Lock, Headset, Bell, HelpCircle, ShieldHalf, Award, ArrowLeft } from "lucide-react";
...
export default function More() {
  ...
  function comingSoon() { setToast("This isn't built in the demo yet"); setTimeout(() => setToast(""), 2200); }
  ...
  return (
    <div className="min-h-dvh flex">
      <Sidebar fullName={fullName} isAdmin={isAdmin} active="more"
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <main className="flex-1 min-w-0 px-5 sm:px-8 py-6 pb-24 lg:py-8 lg:pb-10 overflow-x-hidden">
          ...
          <header className="flex items-start justify-between mb-8">
            <div>
              <h1 className="font-display text-3xl font-bold tracking-tight">Beyond <span className="text-link">Banking</span></h1>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <button onClick={comingSoon} className="text-left lg:col-span-2 bg-paper border border-line rounded-xl p-8 hover:border-slate transition-colors">
              <span className="w-12 h-12 rounded-lg bg-blue/15 flex items-center justify-center mb-3"><Landmark size={20} className="text-blue" /></span>
              <h3 className="text-ink mb-1">Tax Payments</h3>
              <p className="text-slate text-sm">Direct federal and state tax settlements with automatic compliance reporting.</p>
              <p className="num text-link text-sm mt-6">Manage Taxes →</p>
            </button>
            <button onClick={comingSoon} className="text-left bg-paper border border-line rounded-xl p-8 hover:border-slate transition-colors">
              <span className="w-12 h-12 rounded-lg bg-muted-2 flex items-center justify-center mb-3"><ShieldCheck size={20} className="text-slate" /></span>
            ...
            <button onClick={comingSoon} className="text-left bg-paper border-l-4 border-link border-y border-r border-line rounded-xl p-8 hover:border-slate transition-colors">
              <span className="w-12 h-12 rounded-lg bg-muted-2 flex items-center justify-center mb-3"><TrendingUp size={20} className="text-slate" /></span>
            ...
            <button onClick={comingSoon} className="text-left bg-paper border border-line rounded-xl p-8 hover:border-slate transition-colors">
              <span className="w-12 h-12 rounded-lg bg-muted-2 flex items-center justify-center mb-3"><FolderLock size={20} className="text-slate" /></span>
            ...
            <button onClick={comingSoon} className="text-left sm:col-span-2 bg-ink rounded-xl p-8 hover:opacity-90 transition-opacity flex items-center gap-6">
              <span className="w-14 h-14 rounded-full bg-white/10 flex items-center justify-center shrink-0"><Award size={22} className="text-mint" /></span>
              <div>
                ...
                <p className="text-white/60 text-sm">You have 45,200 points available. Redeem for travel, shopping, or cashback.</p>
              </div>
            </button>
            <Link to="/profile" className="bg-paper border border-line rounded-xl p-8 hover:border-slate transition-colors">
              <span className="w-12 h-12 rounded-lg bg-muted-2 flex items-center justify-center mb-3"><ShieldHalf size={20} className="text-slate" /></span>
              <h3 className="text-ink mb-1">Security</h3>
              <p className="text-slate text-sm">Manage biometrics, 2FA, and trusted devices.</p>
            </Link>
            <button onClick={comingSoon} className="text-left bg-ink rounded-xl p-8 hover:opacity-90 transition-opacity">
              <span className="w-12 h-12 rounded-lg bg-white/10 flex items-center justify-center mb-3"><Headset size={20} className="text-white" /></span>
          ...
          <section className="grid sm:grid-cols-3 gap-6 mt-10 items-center">
            <div className="sm:col-span-2">
              ...
              <p className="text-slate text-sm">Your data is protected by bank-grade AES-256 encryption. Our document vault and security hub ensure that your secondary services are as protected as your main accounts.</p>
              <div className="flex gap-3 mt-4">
                <span className="num inline-flex items-center gap-1.5 bg-muted-2 text-ink text-sm px-4 py-2 rounded-full"><Lock size={14} /> End-to-end Encrypted</span>
                <span className="num inline-flex items-center gap-1.5 bg-muted-2 text-ink text-sm px-4 py-2 rounded-full"><ShieldCheck size={14} /> Certified Security</span>
              </div>
            </div>
