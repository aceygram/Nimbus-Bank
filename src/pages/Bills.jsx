...
const CATEGORY_TAGS = {
  ...
  subscription: "bg-muted-2 text-slate",
  insurance: "bg-muted-2 text-slate",
  other: "bg-muted-2 text-slate",
};
    ...
    <div className="bg-paper border border-line rounded-2xl p-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-2">
          <button onClick={() => setCursor(new Date(year, month - 1, 1))} className="text-slate hover:text-ink">
            <ChevronLeft size={14} />
          </button>
          <button onClick={() => setCursor(new Date(year, month + 1, 1))} className="text-slate hover:text-ink">
      ...
      <div className="grid grid-cols-7 gap-1">
        {cells.map((d, i) => (
          <div
            key={i}
            className={`num aspect-square flex items-center justify-center text-[11px] rounded-md relative ${
              d === null ? "" : isToday(d) ? "bg-ink text-white" : dueDaySet.has(d) ? "bg-blue-active text-blue-active-text" : "text-ink"
            }`}
          >
            {d}
            {d !== null && dueDaySet.has(d) && !isToday(d) && (
              <span className="absolute bottom-0.5 w-1 h-1 rounded-full bg-link" />
            )}
          </div>
        ))}
      </div>
...
export default function Bills() {
  ...
  const loadData = useCallback(async (id) => {
    ...
    const { data: billerRows } = await supabase
      .from("billers")
      .select("*")
      .eq("user_id", id)
  ...
  async function handlePayNow(biller) {
    ...
    const { data, error } = await supabase.rpc("pay_bill", {
      p_biller_id: biller.id,
      p_account_id: accounts[0].id,
    });
    setPayingId(null);

    if (error) {
      setToast(error.message.replace(/^.*?:\s*/, ""));
    } else {
      setToast(`Paid ${data.biller_name}`);
      loadData(userId);
    }
    ...
    <div className="min-h-screen flex">
      ...
      <main className="flex-1 px-5 sm:px-8 py-8 pb-24 lg:pb-10 max-w-5xl mx-auto w-full">
        ...
        <div className="grid lg:grid-cols-3 gap-5 mb-8">
          ...
          <div
            className="rounded-3xl p-7 flex flex-col justify-between text-white"
            style={{ backgroundImage: "linear-gradient(135deg, #0B0B0B 0%, #2259BF 100%)" }}
          >
            <div>
              <p className="num text-xs text-white/70 uppercase tracking-wide">Current Balance</p>
            ...
            <button
              ...
              className="num bg-white text-ink text-sm font-medium px-5 py-2.5 rounded-xl mt-4 hover:bg-mist transition-colors self-start"
            >
              + Add New Biller
            </button>
          </div>
            ...
            <section className="bg-paper border border-line rounded-xl p-6">
              <div className="flex items-center justify-between border-b border-line pb-3 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-ink">Upcoming Bills</h3>
                </div>
              ...
              {billers.length === 0 ? (
                <p className="text-sm text-slate text-center py-6">
                  No billers yet. Add one to start tracking payments.
                </p>
              ) : (
                <div className="space-y-3">
                  {billers.map((b) => {
                    const left = daysUntil(b.due_date);
                    const overdue = left <= 2;
                    return (
                        ...
                        <div>
                          ...
                          <p className={`num text-sm mt-0.5 ${overdue ? "text-coral" : "text-slate"}`}>
                            {overdue && left >= 0
                              ...
                              : `Due ${new Date(b.due_date).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}`}
                          </p>
                        </div>
            ...
            <section className="bg-paper border border-line rounded-xl p-6">
              <div className="flex items-center justify-between border-b border-line pb-3 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-ink">Payment History</h3>
                </div>
              </div>

              {history.length === 0 ? (
                <p className="text-sm text-slate text-center py-6">
                  No bills paid yet — payments you make will show up here.
                </p>
              ) : (
                    ...
                    <tbody className="divide-y divide-line">
                      {history.map((tx) => (
                        ...
                        >
                          ...
                          <td className="py-3">
                            <span className={`num text-xs px-3 py-1 rounded-full uppercase ${CATEGORY_TAGS[tx.note] || CATEGORY_TAGS.other}`}>
                              {tx.note || "other"}
                            </span>
                          </td>
          ...
          <div className="space-y-6">
            <section className="bg-paper border border-line rounded-xl p-6">
              <div className="flex items-center gap-2 border-b border-line pb-3 mb-4">
                <Repeat size={16} className="text-ink" />
                <h3 className="text-ink">Autopay</h3>
              </div>

              {autopayBillers.length === 0 ? (
                <p className="text-sm text-slate">No billers on autopay yet.</p>
              ) : (
                <div className="space-y-3">
                  {autopayBillers.map((b) => (
                    <div key={b.id} className="bg-muted rounded-lg p-4 flex items-center justify-between">
                      <div>
                        <p className="text-ink text-sm font-medium">{b.name}</p>
                        <p className="num text-link text-sm">Active</p>
                      </div>
                      <button
                        onClick={() => handleToggleAutopay(b)}
                        className="w-11 h-6 rounded-full bg-link relative transition-colors"
                      >
              ...
              {billers.filter((b) => !b.autopay).length > 0 && (
                <div className="space-y-3 mt-3">
                  {billers.filter((b) => !b.autopay).map((b) => (
                    <div key={b.id} className="bg-muted rounded-lg p-4 flex items-center justify-between">
                      <div>
                        <p className="text-ink text-sm font-medium">{b.name}</p>
                        <p className="num text-slate text-sm">Paused</p>
                      </div>
                      <button
                        onClick={() => handleToggleAutopay(b)}
                        className="w-11 h-6 rounded-full bg-border-soft relative transition-colors"
                      >
            ...
            <section className="bg-muted border border-line rounded-xl p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate text-xs mt-0.5">
                  Your data is protected by Nimbus bank-grade encryption.
                </p>
              </div>
            </section>
          </div>
