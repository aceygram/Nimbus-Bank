  ...
  return (
    <div className="fixed inset-0 bg-ink/40 flex items-center justify-center px-6 z-50">
      <div className="bg-paper rounded-2xl w-full max-w-sm p-6 text-center">
        ...
        <p className="text-slate text-sm mt-2">You'll get a new account number instantly, starting at $0.00.</p>
        {error && <p className="text-sm text-coral bg-coral/10 rounded-lg px-3 py-2 mt-4">{error}</p>}
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 border border-line text-ink font-semibold py-2.5 rounded-full">Cancel</button>
          <button onClick={handleOpen} disabled={saving} className="flex-1 bg-mint text-ink font-bold py-2.5 rounded-full hover:bg-mint-deep transition-colors disabled:opacity-60">
...
export default function Accounts() {
  ...
  const loadData = useCallback(async (id) => {
    ...
    if (accountIds.length > 0) {
      ...
      const { data: txRows } = await supabase.from("transactions").select("*").in("from_account_id", accountIds).gte("created_at", startOfMonth.toISOString());
  ...
  return (
    <div className="min-h-dvh flex">
      <Sidebar fullName={fullName} isAdmin={isAdmin} active="accounts"
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <main className="flex-1 min-w-0 px-5 sm:px-8 py-6 pb-24 lg:py-8 lg:pb-10 overflow-x-hidden">
          <header className="flex items-center justify-between mb-8">
            <div>
              <h1 className="font-display text-3xl font-bold tracking-tight">Your <span className="text-link">Accounts</span></h1>
              <p className="text-slate mt-2 max-w-md">A comprehensive overview of your wealth at Nimbus.</p>
            </div>
          ...
          <div className="bg-paper border border-line rounded-2xl px-6 py-5 shadow-sm mb-6">
            ...
            <p className="num text-3xl font-semibold text-ink mt-1">${totalBalance.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
          </div>
          ...
          <div className="grid sm:grid-cols-3 gap-5 mb-10">
            {accounts.map((a) => {
              ...
              return (
                <div key={a.id} className={`rounded-2xl p-7 flex flex-col justify-between min-h-[180px] ${isSavings ? "text-white" : "bg-paper border border-line"}`}
                  ...
                  <div className="mt-6">
                    ...
                    <p className={`num text-sm mt-1 ${isSavings ? "text-white/60" : "text-slate"}`}>Acc: **** {a.account_number.slice(-4)}</p>
                    <p className="num text-2xl font-bold mt-2">${Number(a.balance).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                    {isSavings && goalPct !== null && (
                      <div className="mt-4">
                        <div className="flex justify-between num text-[11px] text-white/50 uppercase mb-1.5">
                          <span>Goal progress</span><span>{goalPct.toFixed(0)}%</span>
                        </div>
            ...
            {missingTypes.map((m) => (
              <button key={m.type} onClick={() => setOpenModalType(m.type)}
          ...
          <section className="mb-10">
            ...
            <div className="grid sm:grid-cols-4 gap-4">
              <div className="bg-paper border border-line rounded-xl p-5">
                ...
                <p className="text-ink text-xl font-bold mt-2">${totalBalance.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
              </div>
              {wallets.map((w) => (
                <div key={w.id} className="bg-paper border border-line rounded-xl p-5">
                  ...
                  <p className="text-ink text-xl font-bold mt-2">
                    {w.currency === "BTC" ? `${Number(w.balance).toFixed(4)} BTC` : `${w.currency === "EUR" ? "€" : "£"}${Number(w.balance).toLocaleString(undefined, { minimumFractionDigits: 2 })}`}
                  </p>
                </div>
              ))}
            </div>
          ...
          <div className="grid sm:grid-cols-2 gap-5">
                  ...
                  {categoryEntries.map(([cat, amt], i) => (
                    <div key={cat} className="flex items-center justify-between border-b border-line last:border-0 pb-3 last:pb-0">
                      <span className="flex items-center gap-2 text-sm text-ink">
                        <span className={`w-2.5 h-2.5 rounded-full ${dotColors[i % dotColors.length]}`} />{cat}
                      </span>
            ...
            <Link to="/profile" className="bg-muted border border-line rounded-2xl p-6 flex flex-col items-center justify-center text-center hover:border-slate transition-colors">
              ...
              <p className="text-slate text-sm mt-1 max-w-xs">Your accounts are protected by multi-layer encryption and real-time monitoring.</p>
      ...
      {openModalType && (
        <OpenAccountModal type={openModalType} label={MISSING_TYPES.find((m) => m.type === openModalType).label}
          onClose={() => setOpenModalType(null)} onSuccess={handleOpenSuccess} />
      )}
      <MobileTabBar active="accounts" />
    </div>
  );
